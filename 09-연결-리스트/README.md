...
### Floyd’s Cycle-Finding Algorithm (토끼와 거북이 알고리즘)

- 링크드 리스트가 사이클이 있는지(순환되는지)를 탐지하는 알고리즘
- 토끼와 거북이 알고리즘이라고도 한다

아이디어

- 2개의 포인터를 사용한다
  - 거북이 포인터: 느리게 움직임 -> 한 번에 한 단계씩 이동
  - 토끼 포인터: 빠르게 움직임 -> 한 번에 두 단계씩 이동
- 리스트가 순환된다면, 몇 번의 반복 끝에 두 포인터가 결국 같은 곳을 가리키게 됨

복잡도

- 시간 복잡도: O(n)
- 공간 복잡도: O(1)
  - 다른 리스트 필요 없이 두개의 포인터만 사용
